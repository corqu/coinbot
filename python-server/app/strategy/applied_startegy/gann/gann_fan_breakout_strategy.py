...
from app.strategy.basic_startegy.gann.gann_fan import gann_fan_breakout_signal, gann_fan_rays_at
...
def fan_snapshot(
    ...
    rays = gann_fan_rays_at(start=a, end=b, x_index=len(bars) - 1, ratios=ratios)
    if not rays:
        return None
    return {
        "x_index": len(bars) - 1,
...
def generate_signal(
    ...
    return gann_fan_breakout_signal(
        bars=bars,
        start=start,
        end=end,
        ratios=ratios,
