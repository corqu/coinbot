...
def _signal_from_lines(
    ...
    prev_upper_trigger = prev_upper * (1.0 + breakout_buffer)
    curr_upper_trigger = curr_upper * (1.0 + breakout_buffer)
    prev_lower_trigger = prev_lower * (1.0 - breakout_buffer)
    curr_lower_trigger = curr_lower * (1.0 - breakout_buffer)
...
def generate_signal(
    ...
    prev_x = len(bars) - 2
    curr_x = len(bars) - 1
    prev_lines = pitchfork_lines_at((a, b, c), prev_x)
    curr_lines = pitchfork_lines_at((a, b, c), curr_x)
    if prev_lines is None or curr_lines is None:
        return "HOLD"

    prev_close = float(bars[-2]["close"])
    curr_close = float(bars[-1]["close"])
    return _signal_from_lines(
        prev_close=prev_close,
        curr_close=curr_close,
        prev_upper=prev_lines.upper,
        prev_lower=prev_lines.lower,
        curr_upper=curr_lines.upper,
        curr_lower=curr_lines.lower,
