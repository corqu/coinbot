...
export function StrategySettingsPage() {
  ...
  }, [fibCenterPoint, fibEdgePoint, fibRatios, isFibonacciCircles]);
    ...
    <MainLayout>
      <section className="space-y-4">
        <header className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
          ...
          <p className="mt-2 text-sm text-slate-400">왼쪽: 차트, 오른쪽: Python 전략 트리 및 파라미터 입력</p>
              ...
              <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
                ...
                {showParameterEditor ? (
                  ...
                  <div className="mt-3 h-[460px] space-y-3 overflow-y-auto pr-1">
                    ...
                    {activeStrategiesQuery.isError && (
                      <p className="text-xs text-amber-300">전략 목록 조회에 실패했습니다. 백엔드(8080) 연결 상태를 확인해 주세요.</p>
                    )}
                    {!activeStrategiesQuery.isLoading && !activeStrategiesQuery.isError && sortedRoots.length === 0 && (
                      <p className="text-xs text-slate-400">활성화된 전략이 없습니다.</p>
                    )}
